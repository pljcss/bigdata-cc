...
/**
  * 处理craftsman端login数据
  * /home/gt4s/greentown_4s_log/craftsman/login
  *
...
object DealLogGT {
  ...
  val sql = "insert into craftsman_login(user_key,user_id,record_time,widget_key,page_key,from_page_key,version,os,phone_version,os_type,is_app,log_type,status,IP_addr,IP_province,IP_city,mac_addr,operator) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)"
  ...
  def main(args: Array[String]): Unit = {
    ...
    val lines = sc.textFile("C:\\Users\\saicao\\Desktop\\greentown_4s_log\\craftsman\\login\\*")
    ...
    val rr: RDD[Int] = lines.map(a=>
      ...
      b=>
        ...
        ps.setString(13, if (map1.contains("status")) map1.get("status").get else "")
        ps.setString(14, if (map1.contains("IPAddr")) map1.get("IPAddr").get else "")
        ps.setString(15, if (map1.contains("IPProvince")) map1.get("IPProvince").get else "")
        ps.setString(16, if (map1.contains("IPCity")) map1.get("IPCity").get else "")
        ps.setString(17, if (map1.contains("macAddr")) map1.get("macAddr").get else "")
        ps.setString(18, if (map1.contains("operator")) map1.get("operator").get else "")
        ps.executeUpdate()
      //map1.get("IPAddr").get
    }
