...
/**
  ...
  */
object DealLogGtWidget {
  var conn: Connection = null
  var ps: PreparedStatement = null
  val sql = "insert into craftsman_widget(user_key,user_id,record_time,widget_key,page_key,from_page_key,version,os,phone_version,os_type,is_app,log_type,widget) values (?,?,?,?,?,?,?,?,?,?,?,?,?)"
  ...
  def main(args: Array[String]): Unit = {
    ...
    val lines = sc.textFile("C:\\Users\\saicao\\Desktop\\greentown_4s_log\\craftsman\\widget\\*")
    ...
    val rr: RDD[Int] = lines.map(a=>
      ...
      b=>
        ...
        ps.setString(13, if (map1.contains("widget")) map1.get("widget").get else "")
        ps.executeUpdate()
      //map1.get("IPAddr").get
    }
