...
/**
  * Created by saicao on 2017/7/17.
  */
object ToHbaseTest {

  val logger = Logger.getLogger(ToHbaseTest.getClass)
  ...
  private val schemaString = "accs_nbr,label_id,type,flag,load_date"
  ...
  def parseCURLToHbase(date: String)={
    ...
    //val pvRDD = sc.textFile("hdfs://nameservice1/test/c_test.txt")
    ...
    val rowRDD = pvRDD
      .map(_.split("\001"))
      .map(eachRow => Row(eachRow(0), eachRow(1), eachRow(2), eachRow(3), eachRow(4)))
    ...
    for (tel <- arrAD) {
      //logger.info("1111333333666666::::::::开始遍历手机号")
      // 一级标签对应的名称
      ...
      println(tel)
      val sql = "select substring(label_id,0,5),count(substring(label_id,0,5)) from pvTable where accs_nbr=" + "'" + tel + "' group by substring(label_id,0,5)"
      ...
      for (x <- mapLab1) {
        ...
        logger.info("执行到11188888811111行了")
        // 模式匹配
        x._1.toString match {
...
*/
      ...
      //logger.info("22222225555555444444:::::生成了rowkey")
      // 写入HBase
      table.setAutoFlush(false, false) //关键点1
      table.setWriteBufferSize(3 * 1024 * 1024) //关键点2
      put = new Put(Bytes.toBytes(rowkey))

      put.add(Bytes.toBytes("c"), Bytes.toBytes("encryption_tel"), Bytes.toBytes(tel.toString))
      // put值,浏览记录,通信行为
      put.add(Bytes.toBytes("c"), Bytes.toBytes("finance"), Bytes.toBytes(finance))
      put.add(Bytes.toBytes("c"), Bytes.toBytes("sport"), Bytes.toBytes(sport))
      put.add(Bytes.toBytes("c"), Bytes.toBytes("science"), Bytes.toBytes(science))
      put.add(Bytes.toBytes("c"), Bytes.toBytes("life"), Bytes.toBytes(life))
      put.add(Bytes.toBytes("c"), Bytes.toBytes("entertainment"), Bytes.toBytes(entertainment))
      put.add(Bytes.toBytes("c"), Bytes.toBytes("political"), Bytes.toBytes(political))
      put.add(Bytes.toBytes("c"), Bytes.toBytes("shopping"), Bytes.toBytes(shopping))
      put.add(Bytes.toBytes("c"), Bytes.toBytes("reading"), Bytes.toBytes(reading))
      put.add(Bytes.toBytes("c"), Bytes.toBytes("video"), Bytes.toBytes(video))
      put.add(Bytes.toBytes("c"), Bytes.toBytes("music"), Bytes.toBytes(music))
      put.add(Bytes.toBytes("c"), Bytes.toBytes("game"), Bytes.toBytes(game))
      put.add(Bytes.toBytes("c"), Bytes.toBytes("portal"), Bytes.toBytes(portal))
      put.add(Bytes.toBytes("c"), Bytes.toBytes("illegal_site"), Bytes.toBytes(illegal_site))
      put.add(Bytes.toBytes("c"), Bytes.toBytes("region"), Bytes.toBytes(region))
      put.add(Bytes.toBytes("c"), Bytes.toBytes("access_way"), Bytes.toBytes(access_way))
      put.add(Bytes.toBytes("c"), Bytes.toBytes("others"), Bytes.toBytes(others))

      //logger.info("222227777999999::::PUT完毕")
      /*
      // put值,手机号,
